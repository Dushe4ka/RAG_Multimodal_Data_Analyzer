...
_SYSTEM_PROMPT = """Ты полезный ассистент и главный оркестратор.
                    ...
                    Cron (напоминания и периодические задачи):
                    - Если пользователь просит напоминать или создать периодическую задачу (например "каждый понедельник в 10:00"),
                      используй инструменты `cron_add` / `cron_list` / `cron_remove`.
                    - Для периода используй `cron_expr` + `tz` (timezone IANA). Если `tz` не указан — используй `tz="UTC"`.
                    - Для интервала используй `every_seconds` (в секундах).
                    - Для одноразового напоминания используй `at_iso` (ISO datetime, например "2026-02-12T10:30:00+00:00").
                    - Если пользователь говорит "через N секунд/минут/часов" или "один раз" — считай это одноразовым и НЕ используй `every_seconds`/`cron_expr`.
                    - Сделай ровно один вызов `cron_add` и сразу верни подтверждение (не вызывай `cron_add` повторно в рамках одного ответа).
                    - Не пиши напоминания просто в долговременную память — это не создаёт реальных уведомлений.
                    - Во время выполнения cron-задачи (когда входной текст начинается с `[Cron Execution]`) НЕ вызывай инструменты,
...
async def create_agent_with_mcp_async(settings: Settings, memory: AsyncProductionMemory, extra_tools=None):
